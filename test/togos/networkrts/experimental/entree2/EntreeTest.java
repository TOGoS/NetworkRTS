...
public abstract class EntreeTest<EntreeClass extends Entree> extends TestCase
{
	...
	protected int findEntities( long flags, long autoUpdateTime, double x, double y, double w, double h ) throws Exception {
		...
		entree.forEachObject(flags, autoUpdateTime, new TRectangle(x, y, w, h), new Sink<WorldObject>() {
	...
	public void testAddMultipleThingsAndGetThemAndThenRemoveThemAndStuff() throws Exception {
		...
		clear(found);
		entree.forEachObject(0, Long.MAX_VALUE, new TRectangle(0,0,1024,1024), new Sink<WorldObject>() {
		...
		clear(found);
		entree.forEachObject(0, Long.MAX_VALUE, new TRectangle(512,0,1024,1024), new Sink<WorldObject>() {
			@Override public void give(WorldObject p) {
				if( p == o0 ) {
					found[0] = true;
				} else if( p == o1 ) {
