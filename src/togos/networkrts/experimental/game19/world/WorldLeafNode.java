...
public class WorldLeafNode extends BaseWorldNode
{
	...
	static WorldLeafNode create( BlockStack blockStack ) {
	...
	@Override protected WorldNode _update( int x, int y, int size, long time, Message[] messages, List<Action> results ) {
		...
		for( int i=0; i<blockStack.blocks.length; ++i ) {
			newBlocks[i] = blockStack.blocks[i].behavior.update( blockStack.blocks[i], x, y, size, time, messages, results );
